...
CREATE TABLE IF NOT EXISTS cleaning_requests (
    ...
    notified_at     TEXT                     -- when guest was notified of ETA
);
...
-- ============================================================
-- 17. PLACES GUIDE (local attractions, temples, viewpoints)
-- ============================================================
CREATE TABLE IF NOT EXISTS places (
    ...
    description     TEXT,
    distance_km     REAL,
    distance_text   TEXT,                  -- e.g. '5 km', '30 min walk'
    taxi_time_min   INTEGER,               -- estimated taxi time
    bike_time_min   INTEGER,               -- estimated bike time
    icon            TEXT,                  -- emoji icon
    category        TEXT,                  -- 'temple' | 'nature' | 'viewpoint' | 'village' | 'cave' | 'heritage'
...
-- ============================================================
-- 18. PEROOL PRODUCTS (clothing & craft e-commerce)
-- ============================================================
CREATE TABLE IF NOT EXISTS perool_categories (
    id              TEXT PRIMARY KEY,
    name            TEXT NOT NULL,           -- 'Clothing', 'Furnishing', 'Bathroom', etc.
    icon            TEXT,
    sort_order      INTEGER DEFAULT 0,
    is_active       INTEGER DEFAULT 1
);

CREATE TABLE IF NOT EXISTS perool_products (
    id              TEXT PRIMARY KEY,
    category_id     TEXT REFERENCES perool_categories(id),
    name            TEXT NOT NULL,
    description     TEXT,
    icon            TEXT,
    price           REAL,                    -- NULL for enquiry-based
    image_url       TEXT,
    is_available    INTEGER DEFAULT 1,
    sort_order      INTEGER DEFAULT 0
);
...
CREATE INDEX IF NOT EXISTS idx_perool_category ON perool_products(category_id);
CREATE INDEX IF NOT EXISTS idx_perool_available ON perool_products(is_available);
