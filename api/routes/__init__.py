...
def get_perool_products(category_id: str = ""):
    ...
    with get_db() as conn:
        if category_id:
            rows = conn.execute(
                "SELECT * FROM perool_products WHERE category_id = ? AND is_active = 1 ORDER BY sort_order, name",
                (category_id,)
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT * FROM perool_products WHERE is_active = 1 ORDER BY category_id, sort_order, name"
            ).fetchall()
    return [dict(r) for r in rows]
