...
def list_staff():
    staff_list = StaffRepository.list_all()
    # Never return PINs to the client
    return [{**s, "pin": None} for s in staff_list]
...
@router.get("/places")
def get_places():
    """Get all active places/attractions for the guest portal."""
    from api.db import get_db
    with get_db() as conn:
        rows = conn.execute(
            "SELECT * FROM places WHERE is_active = 1 ORDER BY sort_order, name"
        ).fetchall()
...
@router.get("/perool/categories")
def get_perool_categories():
    """Get all active Perool product categories."""
    from api.db import get_db
    with get_db() as conn:
        rows = conn.execute(
            "SELECT * FROM perool_categories WHERE is_active = 1 ORDER BY sort_order, name"
        ).fetchall()
    return [dict(r) for r in rows]


@router.get("/perool/products")
def get_perool_products(category_id: str = ""):
    """Get Perool products, optionally filtered by category."""
    from api.db import get_db
    with get_db() as conn:
        if category_id:
            rows = conn.execute(
                "SELECT * FROM perool_products WHERE category_id = ? AND is_available = 1 ORDER BY sort_order, name",
                (category_id,)
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT * FROM perool_products WHERE is_available = 1 ORDER BY category_id, sort_order, name"
            ).fetchall()
    return [dict(r) for r in rows]
...
def get_staff_contacts():
    """Get staff contacts for the guest portal (name, role, phone, whatsapp, photo)."""
    from api.db import get_db
    with get_db() as conn:
        rows = conn.execute(
            "SELECT id, name, role, phone, whatsapp, photo_url, display_order "
            "FROM staff WHERE is_active = 1 ORDER BY display_order, name"
        ).fetchall()
    return [dict(r) for r in rows]
